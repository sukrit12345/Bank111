...
function viewRefund(refundId, loanId) {
    ...
    window.location.href = `/เพิ่มคืนเงินผ่อนทรัพย์.html?_id=${encodeURIComponent(refundId)}&loan_id=${encodeURIComponent(loanId)}`;
}
...
function divided(refundId, idCardNumber, firstName, lastName, manager, contractNumber, billNumber, initial_profit) {
    ...
    window.location.href = `/เพิ่มส่วนเเบ่งผ่อนทรัพย์.html${queryString}`;
}
